...
description: >-
  Deep UI reference for audio feedback, sound synthesis, predictive prefetch,
  morphing icons, complex presence/container animation, UX laws, or an explicit
  userinterface-wiki review. Use relevant sections only when the user names it
  or a project orchestrator selects one of these niches; do not use as a blanket
  UI baseline.
license: MIT
metadata:
