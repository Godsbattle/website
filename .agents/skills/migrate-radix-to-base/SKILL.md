...
1. Inspect `components.json`, the package manifest, and installed wrappers.
   If CLI output is still needed, use an existing local shadcn CLI or obtain
   approval for an exact version, then run `<shadcn-cli> info --json`.
   Never install or execute `shadcn@latest` automatically.
2. Use Bun for every install and preserve `bun.lock`. Do not invoke npm,
   pnpm, or Yarn.
3. Require a clean git tree and work on a branch. Keep each component change
   reviewable, but never auto-commit.
