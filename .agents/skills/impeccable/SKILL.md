...
name: impeccable
description: >-
  Explicit user-invoked Impeccable design workflow for commands such as craft,
  shape, critique, audit, polish, harden, animate, or live. Use only when the
  user names impeccable or one of its commands. Do not invoke automatically or
  nest beneath trackmyprop create-feature; its mandatory preflight and shape
  gates make it an alternate top-level workflow.
---
